...
public class PlateController : MonoBehaviour
{
    ...
    public float detectionRange = 1.5f; // 꼬치 감지 범위 (시각적으로 표시)
    public float checkInterval = 0.3f; // 검사 주기
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        if (grab == null || grab.isSelected) return; // 잡고 있으면 제외
    ...
    private void DetachSkewer()
    {
        if (attachedSkewer != null)
        {
            ...
            attachedSkewer = null;
            // 물리 상태는 핸들링할 필요 - 잡기 상태 그대로
        }
    }

    // 외부에서 접근 할 수 있는 꼬치 반환 (요리 시스템 등)
