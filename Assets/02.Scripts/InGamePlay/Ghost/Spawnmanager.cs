...
public class SpawnManager : MonoBehaviour
{
    ...
    void SpawnEnemy()
    {
        ...
        if (selectedEnemy != null)
        {
            ...
            {
                Debug.Log($"{selectedEnemy.name} 생성됨 at {spawnPoints[spawnIndex].name}");
            }
        }
    }
    ...
    {
        // spawnChances 배열을 enemyPrefabs와 크기가 다르면 맞춰주기
        if (spawnChances.Length != enemyPrefabs.Length)
        {
            ...
            if (showDebugMessages)
            {
                Debug.Log("SpawnChances 배열 크기를 EnemyPrefabs에 맞게 조정했습니다.");
            }
        }
    }
